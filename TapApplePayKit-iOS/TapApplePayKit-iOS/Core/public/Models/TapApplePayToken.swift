...
@objcMembers public class TapApplePayToken:NSObject {
    ...
    var rawAppleToken:PKPaymentToken? {
        didSet{
    ...
    internal func convertTokenToString(){
        ...
        if let nonNullAppleToken = self.rawAppleToken {
            stringAppleToken = String(data: nonNullAppleToken.paymentData, encoding: .utf8) ?? ""
            if stringAppleToken == ""
            {
